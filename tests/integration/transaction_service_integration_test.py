...
class TestTransactionService:
    ...
    async def _setup(self, transaction_service):
        ...
        transactions = await self.service.get_user_transactions()
        if transactions:
            self.test_transaction_id = transactions[0].id
            self.test_transaction_reference = transactions[0].reference

    async def test_get_user_transactions(self):
        txs = await self.service.get_user_transactions()
    ...
    async def test_get_transaction_by_id(self):
        ...
        tx = await self.service.get_transaction(self.test_transaction_id)
        logger.info(f"Transaction by ID: {tx}")

        assert tx.id == self.test_transaction_id

    async def test_get_transaction_by_reference(self):
        if not self.test_transaction_reference:
            pytest.skip("No transaction available to test get_transaction_by_reference")

        tx = await self.service.get_transaction_by_reference(
